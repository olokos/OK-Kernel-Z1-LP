    ...
    while (count) {
        ...
        case 2:
            if (sense[0] & 0x30) {
                ...
                case 0:
                    printk("drive error, code = 0x%X",sense[0] & 0x0F);
                    break;
                case 1:
                    printk("controller error, code = 0x%X",sense[0] & 0x0F);
                    break;
                case 2:
                    printk("command error, code = 0x%X",sense[0] & 0x0F);
                    break;
                case 3:
                    printk("miscellaneous error, code = 0x%X",sense[0] & 0x0F);
                    break;
                }
            }
            if (sense[0] & 0x80)
...
static void __init xd_dtc_init_controller (unsigned int address) {
    ...
    case 0xD8000:
        break;			/*5150CX & 5150XL*/
    default:
        printk("xd_dtc_init_controller: unsupported BIOS address %06x\n",address);
        break;
    }
...
static void __init xd_seagate_init_controller (unsigned int address) {
    ...
    case 0xE0000:
        ...
        break;
    default:
        printk("xd_seagate_init_controller: unsupported BIOS address %06x\n",address);
        break;
    }
    xd_maxsectors = 0x40;
...
static void __init do_xd_setup (int *integers) {
    ...
    case 0:
        break;
    default:
        printk("xd: too many parameters for xd\n");
    }
    xd_maxsectors = 0x01;
}
